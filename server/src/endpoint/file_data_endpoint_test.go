...
var FILE_DATA_QUERY string = "SELECT id, filename, TO_CHAR(creation_date, 'YYYY.MM.DD HH24:MI:SS') AS date, mime_type, size, CASE WHEN thumbnail IS NOT NULL THEN thumbnail ELSE file END FROM files WHERE user_id = $1 AND TO_CHAR(creation_date, 'YYYY.MM.DD') ILIKE $2 || '%' ORDER BY creation_date DESC, id DESC"
var OFFSET int64 = 10
var COUNT int64 = 15
